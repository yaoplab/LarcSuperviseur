...
from PySide6.QtCore import QCoreApplication, QDate, QDateTime, Qt, QTime
...
class GroupPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        self._history_filter_type.lineEdit().setPlaceholderText(_("history.filter_placeholder"))
        filter_row.addWidget(M3Label(_("history.filter_type")))
        filter_row.addWidget(self._history_filter_type)
        filter_row.addSpacing(10)
        filter_btn = M3Button(_("common.button.filter"))
        ...
        self._charts_tabs.addTab(tab_abs, _("chart.serie_absences"))
        ...
        self._charts_tabs.addTab(tab_exit, _("chart.serie_exits"))
        ...
        stats_title = M3Label(f"<b>{_('chart.stats_by_class')}</b>")
    ...
    def load(self, mode: str, date_from: str, date_to: str):
        ...
        self._set_loading(True, _("group_panel.loading_stats"))
        try:
    ...
    def _update_everything(self, mode, date_from, date_to):
        ...
        if rows:
            ...
            self._abs_bar.setTitle(_("chart.abs_by_class"))
            ...
            self._abs_bar.setTitle(_("chart.abs_by_class_nodata"))
        ...
        if rows:
            ...
            self._exit_bar.setTitle(_("chart.exit_by_class"))
            ...
            self._exit_bar.setTitle(_("chart.exit_by_class_nodata"))
        ...
        if trend_rows:
            ...
            self._trend_chart.setTitle(_("chart.abs_trend"))
            self._trend_chart.setAnimationOptions(QChart.SeriesAnimations)
            ...
            self._trend_chart.setTitle(_("chart.abs_trend_nodata"))
        ...
        if present_count > 0 or absent_count > 0:
            ...
            if present_count > 0:
                donut.append(_("chart.present_slice"), present_count)
                donut.slices()[-1].setColor(QColor(p.success))
                donut.slices()[-1].setLabelVisible(True)
                donut.slices()[-1].setLabel(f"{_('chart.present_slice')} {present_count}")
                donut.slices()[-1].setLabelColor(QColor(p.text_strong))
            if absent_count > 0:
                donut.append(_("chart.absent_slice"), absent_count)
                donut.slices()[-1].setColor(QColor(p.error))
                donut.slices()[-1].setLabelVisible(True)
                donut.slices()[-1].setLabel(f"{_('chart.absent_slice')} {absent_count}")
                donut.slices()[-1].setLabelColor(QColor(p.text_strong))
            ...
            self._donut_chart.setTitle(_("chart.presence_rate_nodata"))
    ...
    def _load_history(self, date_from: str, date_to: str):
        if not self._term_id:
            return
        self._set_loading(True, _("group_panel.loading_events"))
        try:
            ...
            rows = self._loader.get_event_history(
                self._current_mode, date_from, date_to, class_id=sel_class, type_filter=sel_type
            )
    ...
    def _set_loading(self, busy: bool, msg: str = _("loading_message")):
    ...
    def _get_event_id_from_table(self, table: M3TableWidget):
